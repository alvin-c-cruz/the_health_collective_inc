    ...
    def _post(self, request_form, current_user_id):
        for attribute in get_attributes(self):
            if attribute == "id":
                ...
                attribute_value = request_form.get("account_class_name")
                account_class = AccountClass.query.filter(
                    AccountClass.account_class_name == attribute_value
                ).first()
                print(account_class)
                self.account_class_name = account_class.account_class_name
    ...
    def _validate_on_submit(self):
        ...
        if not self.account_type_name:
            ...
            duplicate = Obj.query.filter(
                func.lower(Obj.account_type_name) == func.lower(self.account_type_name),
                Obj.id != self.id,
            ).first()
            if duplicate:
        ...
        if not self.account_class_name:
            ...
            existing = AccountClass.query.filter(
                func.lower(AccountClass.account_class_name)
