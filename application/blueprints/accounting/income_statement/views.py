...
def home():
    ...
    for account in accounts:
        ...
        beginning_balance = (
            account.balance(from_date - timedelta(days=1)) if from_date else 0
        )
        ...
        account_data = {
            ...
            "debit": account.debit_balance(to_date)
            - account.debit_balance(from_date - timedelta(days=1))
            if from_date
            else account.debit_balance(to_date),
            "credit": account.credit_balance(to_date)
            - account.credit_balance(from_date - timedelta(days=1))
            if from_date
            else account.credit_balance(to_date),
        }
...
def download_excel():
    ...
    for account in accounts:
        ...
        if "REVENUE" in class_name or "INCOME" in class_name or "SALES" in type_name:
            credit = account.credit_balance(to_date) - account.credit_balance(
                from_date - timedelta(days=1)
            )
            debit = account.debit_balance(to_date) - account.debit_balance(
                from_date - timedelta(days=1)
            )
            amount = credit - debit
            if amount != 0:
