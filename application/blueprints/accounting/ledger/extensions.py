...
align_center_center_wrap = Alignment(
            ...
            for entry in voucher.entries:
                if entry.account == account:
                    _transaction = {
                        ...
                        "view_url": url_for(module_name[obj] + ".view", id=voucher.id)
                        if hasattr(voucher, "date_posted")
                        else "",
                    }
...
def create_ledger_all(accounts, date_from, date_to, instance_path):
    ...
    for account in accounts:
        ...
        wtb_row_num = write_trial_balance(
            ws_trial_balance, wtb_row_num, ws, row_num, account
        )
...
def write_header(ws, row_num, account, date_from, date_to):
    ...
    cell = ws[f"A{row_num}"]
    cell.value = (
        f"From {date_from.strftime('%B %d, %Y')} to {date_to.strftime('%B %d, %Y')}"
    )
    cell.font = font_10
    row_num += 3
        ...
        for column_letter, value in columns.items():
            ...
            if column_letter in ("D", "L"):
                if transaction["date_posted"]:
                    cell.hyperlink = (
                        "https://jasmincy11.pythonanywhere.com"
