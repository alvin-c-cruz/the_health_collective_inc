...
def home():
    ...
    if True:
        ...
        for obj in (
            ...
            vouchers = obj.query.filter(
                obj.record_date >= date_from, obj.record_date <= date_to
            ).all()
            for voucher in vouchers:
                for entry in voucher.entries:
                    if entry.account.account_number == account_number.split(":")[0]:
                        transaction = {
                            ...
                            "bank_date": voucher.bank_date
                            if hasattr(voucher, "bank_date")
                            else voucher.record_date,
                            "book": book[obj],
                            "reference": voucher.record_number,
                            "particulars": voucher.notes,
                            "debit": entry.debit,
                            "credit": entry.credit,
                            "formatted_debit": f"{entry.debit:,.2f}",
                            "formatted_credit": f"{entry.credit:,.2f}",
                            "date_posted": voucher.date_posted
                            if hasattr(voucher, "date_posted")
                            else "",
                            "edit_url": url_for(
                                module_name[obj] + ".edit", id=voucher.id
                            ),
                            "view_url": url_for(
                                module_name[obj] + ".view", id=voucher.id
                            )
                            if hasattr(voucher, "date_posted")
                            else "",
                        }
    ...
    if cmd_button == "Download":
        account = Accounts.query.filter(
            Accounts.account_number == account_number.split(":")[0]
        ).first()
        filename = create_ledger(
            account,
            beginning,
            sorted_transactions,
            date_from,
    ...
    if cmd_button == "Download All":
        accounts = Accounts.query.all()
        filename = create_ledger_all(
            accounts, date_from, date_to, current_app.instance_path
        )
        return send_file(f"{filename}", as_attachment=True)
