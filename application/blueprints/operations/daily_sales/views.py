...
@bp.route('/transaction_types', methods=['GET'])
@login_required
@roles_accepted([ROLES_ACCEPTED])
def transaction_type_list():
    types = TransactionType.query.order_by(TransactionType.sort_order).all()
    return render_template('daily_sales/transaction_types/home.html',
                           types=types, app_label=app_label)


@bp.route('/transaction_types/add', methods=['GET', 'POST'])
@login_required
@roles_accepted([ROLES_ACCEPTED])
def transaction_type_add():
    ...
    if request.method == 'POST':
        ...
        elif TransactionType.query.filter_by(type_code=obj.type_code).first():
        ...
        if not errors:
            ...
            return redirect(url_for('daily_sales.transaction_type_list'))

    return render_template('daily_sales/transaction_types/form.html',
                           obj=obj, errors=errors, is_new=True, app_label=app_label)


@bp.route('/transaction_types/<int:type_id>/edit', methods=['GET', 'POST'])
@login_required
@roles_accepted([ROLES_ACCEPTED])
def transaction_type_edit(type_id):
    from application.extensions import db
    obj = TransactionType.query.get_or_404(type_id)
    errors = {}
    if request.method == 'POST':
        ...
        if not new_code:
            ...
            if TransactionType.query.filter_by(type_code=new_code).first():
                errors['type_code'] = 'This type code already exists.'
            else:
...
@bp.route('/transaction_types/<int:type_id>/toggle', methods=['POST'])
@login_required
@roles_accepted([ROLES_ACCEPTED])
def transaction_type_toggle(type_id):
    from application.extensions import db
    obj = TransactionType.query.get_or_404(type_id)
