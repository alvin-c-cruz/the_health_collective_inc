...
def _snapshot_transaction(transaction):
    ...
    snapshot = {
        ...
        "transaction_type_name": transaction.transaction_type.type_name
...
def request_transaction_change(transaction_id):
    ...
    print(
        f"DEBUG: request_transaction_change called for transaction_id={transaction_id}",
        file=sys.stderr,
    )
    ...
    if user:
        ...
        print(
            f"DEBUG: hasattr(user, 'is_staff')={hasattr(user, 'is_staff')}",
            file=sys.stderr,
        )
        try:
        ...
        print(
            f"DEBUG: can_request_changes(user)={can_request_changes(user)}",
            file=sys.stderr,
        )
    else:
    ...
    if not transaction.submitted:
        flash(
            "Can only request changes on submitted or approved transactions.", "warning"
        )
        return redirect(
            url_for("daily_sales.view_transaction", transaction_id=transaction_id)
        )
    ...
    if existing_request:
        flash(
            "There is already a pending modification request for this transaction.",
            "warning",
        )
        return redirect(
            url_for("daily_sales.view_transaction", transaction_id=transaction_id)
    ...
    if request.method == "POST":
        ...
        new_snapshot = {
            ...
            "record_number": transaction.record_number or "",
            "dashlabs_number": request.form.get(
                "dashlabs_number", transaction.dashlabs_number or ""
            ),
            "pos_number": request.form.get("pos_number", transaction.pos_number or ""),
            "customer_id": int(
                request.form.get("customer_id", transaction.customer_id)
            ),
                ...
                details.append(
                    {
                        ...
                        "discount": float(detail_discounts[i])
                        if i < len(detail_discounts) and detail_discounts[i]
                        else 0.0,
                ...
                tenders.append(
                    {
                        ...
                        "amount": float(tender_amounts[i])
                        if i < len(tender_amounts) and tender_amounts[i]
                        else 0.0,
...
def change_requests_list():
    ...
    submitted_transactions = (
        ...
        .order_by(Transaction.submitted.desc())
        .all()
    )
    ...
    submitted_deposits = (
        Deposit.query.filter_by(status="submitted")
        .order_by(Deposit.submitted_at.desc())
        .all()
    )
    ...
    submitted_collections = (
        Collection.query.filter_by(status="submitted")
    ...
    if action == "approve":
        ...
        if change_request.is_cancellation:
            ...
            if record:
                ...
                flash(
                    "Cancellation request approved. Record has been cancelled.",
                    "success",
                )
            else:
            ...
            if transaction:
                ...
                transaction.record_date = new_vals.get(
                    "record_date", transaction.record_date
                )
                transaction.dashlabs_number = new_vals.get(
                    "dashlabs_number", transaction.dashlabs_number
                )
                transaction.pos_number = new_vals.get(
                    "pos_number", transaction.pos_number
                )
                transaction.customer_id = new_vals.get(
                    "customer_id", transaction.customer_id
                )
                transaction.description = new_vals.get(
                ...
                TransactionDetail.query.filter_by(
                ...
                TransactionTender.query.filter_by(
                    transaction_id=transaction.id
                ).delete()
                ...
                flash(
                    "Modification request approved. Changes have been applied.",
                    "success",
                )
            else:
...
def change_history():
    ...
    """
    history = (
        ChangeRequest.query.filter(ChangeRequest.status != "pending")
        .order_by(ChangeRequest.requested_at.desc())
        .all()
    )

    return render_template("daily_sales/change_history.html", history=history)
...
def transaction_history(transaction_id):
    ...
    change_log = (
        ChangeRequest.query.filter_by(transaction_id=transaction_id)
        .order_by(ChangeRequest.requested_at.asc())
        .all()
    )
...
def request_transaction_cancellation(transaction_id):
    ...
    if existing_request:
        flash(
            "There is already a pending cancellation request for this transaction.",
            "warning",
        )
        return redirect(
            url_for("daily_sales.view_transaction", transaction_id=transaction_id)
    ...
    if request.method == "POST":
        ...
        if not reason:
            ...
            return render_template(
                "daily_sales/request_cancellation.html", transaction=transaction
            )
