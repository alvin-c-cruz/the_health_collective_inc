...
from ..daily_sales.models import Transaction, TransactionTender
...
def _receivable_tenders():
    return (
        Tender.query.filter_by(is_receivable=True)
        .order_by(Tender.sort_order.desc())
        .all()
    )
...
def _outstanding_lines(tender_id=None, ape_batch_id=None):
    ...
    query = (
        TransactionTender.query.filter(TransactionTender.tender_id.in_(receivable_ids))
        .join(TransactionTender.transaction)
        .filter(
            db.text(
                '"transaction".submitted IS NOT NULL'
            ),  # Include all submitted transactions
...
def new_collection():
        ...
        if not tender_id:
            ...
            for lid, lamt in zip(line_ids, line_amounts):
                ...
                if txn_tender and txn_tender.transaction:
                    ...
                    txn_list.append(
                        f"#{txn.record_number} - {txn.customer.customer_name if txn.customer else 'No customer'} (₱{amt:,.2f})"
                    )
            ...
            db.session.commit()
            flash(
                f"Collection saved as draft. Net amount: ₱{col.formatted_net_amount}",
                "success",
            )
    ...
    lines = (
        _outstanding_lines(tender_id_pre, ape_batch_id_pre)
        if (tender_id_pre or ape_batch_id_pre)
        else []
    )
    outstanding_rows = []
    for tt in lines:
...
def edit_collection(collection_id):
    ...
    if request.method == "POST":
        ...
        if not tender_id:
            ...
            db.session.commit()
            flash(
                f"Collection updated. Net amount: ₱{col.formatted_net_amount}",
                "success",
            )
            return redirect(
                url_for(f"{app_name}.view_collection", collection_id=col.id)
            )
    ...
    for tt in lines:
        collected = _collected(
            tt.id, exclude_collection_id=col.id
        )  # Exclude current collection
        outstanding = tt.amount - collected
        if (
            outstanding > 0 or tt.id in existing_details
        ):  # Include if has outstanding OR already in this collection
...
def delete_collection(collection_id):
    ...
    record_id = col.id
    identifier = (
        f"Collection #{col.id} - {col.tender.tender_name if col.tender else 'N/A'}"
    )
...
def submit_collection(collection_id):
    ...
    if col.status != "draft":
        ...
        return redirect(
            url_for(f"{app_name}.view_collection", collection_id=collection_id)
        )
    ...
    for detail in col.details:
        ...
        txn = txn_tender.transaction
        txn_list.append(
            f"#{txn.record_number} - {txn.customer.customer_name if txn.customer else 'No customer'} (₱{detail.formatted_amount})"
        )
    ...
    flash(
        f"Collection submitted successfully! Waiting for approval. Net amount: ₱{col.formatted_net_amount}",
        "success",
    )
...
def approve_collection(collection_id):
    ...
    flash(
        f"Collection approved and posted! Total amount: ₱{col.formatted_total_amount}",
        "success",
    )
    return redirect(url_for(f"{app_name}.view_collection", collection_id=collection_id))
...
def cancel_collection(collection_id):
    ...
    if request.method == "POST":
        ...
        if not reason:
            ...
            return redirect(
                url_for(f"{app_name}.cancel_collection", collection_id=collection_id)
            )
...
def request_collection_change(collection_id):
    ...
    if col.status not in ["submitted", "posted"]:
        flash(
            "Only submitted or posted collections can have change requests.", "danger"
        )
        return redirect(
            url_for(f"{app_name}.view_collection", collection_id=collection_id)
        )
    ...
    if request.method == "POST":
        ...
        if not reason:
            ...
            return redirect(
                url_for(
                    f"{app_name}.request_collection_change", collection_id=collection_id
                )
            )
...
def request_collection_cancellation(collection_id):
    ...
    if existing_request:
        flash(
            "There is already a pending cancellation request for this collection.",
            "warning",
        )
        return redirect(
            url_for(f"{app_name}.view_collection", collection_id=collection_id)
    ...
    if request.method == "POST":
        ...
        if not reason:
            ...
            return render_template(
                "collections/request_collection_cancellation.html", **context
            )
...
def collection_change_requests():
    ...
    change_requests = (
        ChangeRequest.query.filter(
            ChangeRequest.record_type == "collection", ChangeRequest.status == "pending"
        )
...
def review_collection_change_request(cr_id):
    ...
    if action == "approve":
        ...
        cr.status = "approved"
        flash(
            "Change request approved. Collection updated and set to submitted status for re-approval.",
            "success",
        )
