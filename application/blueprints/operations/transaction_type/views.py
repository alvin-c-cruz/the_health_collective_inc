...
from application.blueprints.user import current_user, login_required, roles_accepted
...
def home():
    ...
    counts = dict(
        ...
        .group_by(Transaction.transaction_type_id)
        .all()
    )
    return render_template(
        "transaction_type/home.html",
        types=types,
        app_label=app_label,
        txn_counts=counts,
    )
...
def add():
    ...
    if request.method == "POST":
        obj.type_code = (
            request.form.get("type_code", "").strip().lower().replace(" ", "_")
        )
        ...
        if not errors:
            ...
            for tender in tenders:
                ...
                existing_types = [
                    t for t in existing_types if t
                ...
                elif obj.type_code in existing_types:
                    existing_types.remove(obj.type_code)

                tender.transaction_types = (
                    ",".join(existing_types) if existing_types else None
                )
            ...
            try:
                ...
                flash(
                    f"Transaction type saved, but audit logging failed: {e!s}",
                    "warning",
                )
    ...
    if request.method == "POST":
        ...
        if not errors:
            ...
            for tender in tenders:
                ...
                # Remove this type if present
                elif new_code in existing_types:
            ...
            try:
                ...
                flash(
                    f"Transaction type updated, but audit logging failed: {e!s}",
                    "warning",
                )
                print(f"Audit logging failed: {e}")
...
def toggle(record_id):
    ...
    try:
        ...
        flash(f"Status changed, but audit logging failed: {e!s}", "warning")
        print(f"Audit logging failed: {e}")
...
def delete(record_id):
    ...
    if count > 0:
        flash(
            f'Cannot delete "{obj.type_name}" — it has {count} linked transaction(s).',
            "danger",
        )
        return redirect(url_for("transaction_type.home"))
    ...
    try:
        ...
        flash(
            f"Transaction type deleted, but audit logging failed: {e!s}", "warning"
        )
        print(f"Audit logging failed: {e}")
