    ...
    def _save(self):
        ...
        if self.id is None:
            obj = Obj(
                ...
                else f"{self.bank_name} — {self.account_number}",
                notes=self.notes,
                active=self.active,
            )
            db.session.add(obj)
        ...
        try:
            if is_new:
                ...
                new_values = model_to_dict(
                    obj,
                    ["bank_name", "account_name", "account_number", "notes", "active"],
                )
                log_update(
                    module="bank_account",
                    record_id=obj.id,
            ...
            flash(f"Bank account saved, but audit logging failed: {e!s}", "warning")
