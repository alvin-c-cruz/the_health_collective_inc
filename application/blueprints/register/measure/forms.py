...
from .admin_models import UserMeasure as Preparer
...
class Form:
    ...
    def _save(self):
        if self.id is None:
            ...
            log_create(
                ...
                new_values=model_to_dict(new_record, ['measure_name']),
                notes='Measure created'
            )
            ...
            if record:
                ...
                old_values = model_to_dict(record, ['measure_name'])
                ...
                new_values = model_to_dict(record, ['measure_name'])
    ...
    def _validate_on_submit(self):
        ...
        if not self.measure_name:
            self.errors["measure_name"] = "Please type description."
        else:
            duplicate = Obj.query.filter(
                func.lower(
                    Obj.measure_name
                    ) == func.lower(self.measure_name), 
                    Obj.id != self.id
                    ).first()
            if duplicate:
                self.errors["measure_name"] = "Measure is already used."               
