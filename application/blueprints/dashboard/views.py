...
def home():
    ...
    context["get_sorted_items"] = (
        get_sorted_items  # Make helper function available in template
    )
    ...
    try:
        ...
        for index, item_key in enumerate(items):
            sort_order = ReportItemSortOrder(
                section=section, item_key=item_key, sort_order=index
            )
            db.session.add(sort_order)

        db.session.commit()
        return jsonify(
            {"success": True, "message": f"Sort order saved for {len(items)} items"}
        )
...
def get_sort_order(section):
    ...
    try:
        orders = (
            ReportItemSortOrder.query.filter_by(section=section)
            .order_by(ReportItemSortOrder.sort_order)
            .all()
        )
