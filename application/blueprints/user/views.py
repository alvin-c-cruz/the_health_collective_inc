...
def user_superuser():
    if not current_user.superuser:
        flash(
            "Only a SuperAdmin can grant or revoke SuperAdmin status.", category="error"
        )
...
def toggle_superuser():
    ...
    if user.user_name != "admin":
        ...
        db.session.commit()
        flash(
            f"SuperUser status {'enabled' if value else 'disabled'} for {user.user_name}",
            category="success",
        )
    else:
        flash("Cannot change super admin status", category="error")
...
def toggle_admin():
    ...
    if user.user_name != "admin":
        ...
        db.session.commit()
        flash(
            f"Administrator status {'enabled' if value else 'disabled'} for {user.user_name}",
            category="success",
        )
    else:
        flash("Cannot change super admin status", category="error")
...
def toggle_staff():
    ...
    db.session.commit()
    flash(
        f"Staff status {'enabled' if value else 'disabled'} for {user.user_name}",
        category="success",
    )
...
def toggle_viewer():
    ...
    db.session.commit()
    flash(
        f"Viewer status {'enabled' if value else 'disabled'} for {user.user_name}",
        category="success",
    )
    ...
    if request.method == "POST":
        ...
        if form.validate():
            ...
            if user:
                    ...
                    if current_app.config.get("MAINTENANCE_MODE", False):
                        if not user.superuser:
                            ...
                            log_login(
                                user,
                                status="failed",
                                failure_reason="Site in maintenance mode",
                            )
                ...
                log_login(
                    form.user_name, status="failed", failure_reason="User not found"
                )
...
def change_password():
    ...
    if request.method == "POST":
        ...
        if not user:
            ...
            flash("Password has changed.", category="success")
...
def check_roles():
    ...
    for module in modules:
        if hasattr(module, "menu_label"):
            expected.add(module.menu_label[2])
...
def user_management():
    ...
    if not current_user_obj or not (
        current_user_obj.is_superuser or current_user_obj.is_admin
    ):
...
def update_user_roles(user_id):
    ...
    if user.is_admin and not request.form.get("is_admin"):
        flash(
            f"User '{user.user_name}' is a protected admin and cannot be demoted.",
            "danger",
        )
        return redirect(url_for("user.user_management"))
