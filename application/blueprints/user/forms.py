    ...
    def validate(self):
        ...
        if not self.user_name:
            ...
            user = User.query.filter(
                func.upper(User.user_name) == func.upper(self.user_name),
                User.id != self.id,
            ).first()
            if user:
...
class LoginForm:
    ...
    def post(self, request_form):
        self.user_name = (
            request_form.get("user_name") if request_form.get("user_name") else ""
        )
        self.pass_word = (
            request_form.get("pass_word") if request_form.get("pass_word") else ""
        )
